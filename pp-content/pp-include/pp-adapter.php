...
<?php
        ...
        if($action == ""){
            ...
            if($action == "general-setting"){
                if($global_user_login == true){
                    ...
                    $verification_method = escape_string($_POST['verification_method'] ?? 'transaction_id');
                    ...
                    if($autoExchange == ""){
                        ...
                        $columns = ['autoExchange', 'favicon', 'logo', 'name', 'timezone', 'language', 'currency_code', 'payment_tolerance', 'verification_method', 'street_address', 'city_town', 'postal_code', 'country', 'support_phone_number', 'support_email_address', 'support_website', 'whatsapp_number', 'telegram', 'facebook_messenger', 'facebook_page', 'updated_date'];
                        $values = [$autoExchange, $branding_favicon, $branding_primary_logo, $site_name, $default_timezone, $default_language, $default_currency, money_sanitize($payment_tolerance), $verification_method, $street_address, $city_town, $postal_code, $country, $support_phone_number, $support_email_address, $support_website, $whatsapp_number, $telegram, $facebook_messenger, $facebook_page, getCurrentDatetime('Y-m-d H:i:s')];
    ...
    if(isset($_POST['action-v2'])){
        ...
        if($action == ""){
            ...
            if($action == "pp-phone-poll"){
                echo pp_phone_handle_poll();
                exit();
            }
            if($action == "pp-phone-verify"){
                echo pp_phone_handle_verify();
                exit();
            }
            if($action == "pp-phone-cancel"){
                echo pp_phone_handle_cancel();
                exit();
            }
