<?php
    ...
    // Phone verification (external file)
    if (file_exists(__DIR__.'/phone-verify.php')) {
        require_once __DIR__.'/phone-verify.php';
    }
        ...
        if($response_gateway['status'] == true){
            ...
            if(isset($gateway_info)){
                if(isset($gateway_info['gateway_type']) && $gateway_info['gateway_type'] == "automation"){
                    ...
                    // Check if brand uses phone verification — fetch directly from DB
                    $vm_brand_id = $data['brand']['id'] ?? '';
                    $vm_result = json_decode(getData($db_prefix.'brands', 'WHERE brand_id = :brand_id', 'verification_method FROM', [':brand_id' => $vm_brand_id]), true);
                    $verification_method = $vm_result['response'][0]['verification_method'] ?? 'transaction_id';

                    if($verification_method === 'phone_number'){
                        // Phone verification: 2-step wizard
                        pp_phone_render_form($data);
                    } else {
                        // Legacy: Transaction ID form
                        echo '
